...
# and put here its credentials (-u and -p options). See below for details.
# Blankspaces matter.
PARAMS= -sdefault -u mailprocess -p password -e Job -a process_mailing
PARAMSBOUNCE= -sdefault -u mailprocess -p password -e Job -a fetch_bounces
...
1. The site you use, which is **-sdefault**. This parameter is optional if you do not run multiple CiviCRM sites. If you run multiple CiviCRM sites (applicable for Drupal) on a single server, you need to specify your site's domain, such as **-sexample.org**.
